...
@dp.callback_query_handler(
    IsSuperAdmin(),
    discipline_callback.filter(action=DisciplineActionEnum.view_sub_category.value),
)
@session_decorator(add_param=False)
async def discipline_list(cq: types.CallbackQuery) -> None:
    kb = await get_discipline_kb()
    await cq.message.answer("Меню дисциплін", reply_markup=kb)
    await cq.answer()
...
async def load_name(msg: types.Message, state: FSMContext):
    ...
    await msg.reply("Ведіть опис дисципліни:")
...
async def discipline_edit_btn(
    ...
    await cq.message.answer(
        f"Оберіть що хочете відредагувати в «{discipline.name}»",
        reply_markup=edit_discipline_kb(int(callback_data["discipline_id"])),
    )


@dp.callback_query_handler(
    IsSuperAdmin(),
    discipline_edit_callback.filter(action=DisciplineActionEnum.edit_name.value),
)
@session_decorator()
...
@dp.callback_query_handler(
    IsSuperAdmin(),
    discipline_edit_callback.filter(
...
async def discipline_edit_short_description(
    ...
    await cq.message.answer(
        "Введіть новий короткий опис дисципліни \nДля відміни - /cancel"
    )
