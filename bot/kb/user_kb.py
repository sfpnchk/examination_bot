...
discipline_statistics_callback = CallbackData("discipline_statistics", "discipline_id")
    ...
    kb.row(
        InlineKeyboardButton(
            ...
            callback_data=question_callback.new(
                action=QuestionActionEnum.view.value, discipline_id=discipline_id
            ),
        ),
        InlineKeyboardButton(
            "Назад до вибору дисциплін",
            callback_data=discipline_callback.new(
                action=DisciplineActionEnum.view_list.value, discipline_id=0
            ),
        ),
    )
    return kb
...
def get_main_menu_kb():
    ...
    kb.add(InlineKeyboardButton("Дисципліни"), InlineKeyboardButton("Статистика"))
    return kb


def get_discipline_for_statistics(disciplines):
    kb = InlineKeyboardMarkup()
    for discipline in disciplines:
        kb.add(
            InlineKeyboardButton(
                discipline.name,
                callback_data=discipline_statistics_callback.new(
                    discipline_id=discipline.id
                ),
            )
        )
